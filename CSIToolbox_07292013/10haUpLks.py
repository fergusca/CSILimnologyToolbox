...
tenhalakes = os.path.join(mem, "tenhalakes")



# Spatial join cumulative watersheds to 10 hectare and larger lakes
# Field mapping:
...
fm_areasqkm.addInputField(tenhalakes, "AreaSqKm")
...
# Spatial Join
arcpy.SpatialJoin_analysis(cws, tenhalakes, os.path.join(outfolder, "CalcTenHaLk" + subregion + ".shp"), '', '', fms, "HAVE_THEIR_CENTER_IN")

sj = os.path.join(outfolder, "CalcTenHaLk" + subregion + ".shp")

# Change name of join_count to T10ha (the total 10 ha lakes with their center in that cumulative watershed)
