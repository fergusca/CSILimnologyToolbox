...
##############main##########################################################################
def main():
    fcs = arcpy.ListFeatureClasses(mem)
    fc_list = [os.path.join(mem,fc) for fc in fcs]
    pool = multiprocessing.Pool()
    pool.map(zsta,fc_list,raster)
    pool.close()
    pool.join()
...
arcpy.AddMessage("Finished multiprocessing. Starting merge.")
